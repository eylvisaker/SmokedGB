	...
	public abstract class MemoryController
	{
		...
		private void WriteToHighMem(int address, byte value)
		{
			switch (address)
			{
				...
				case 0xff13:
				case 0xff14:
					theGameboy.aud.SoundWrite(address);
					break;
				...
				case 0xff3e:
				case 0xff3f:
					theGameboy.aud.SoundWrite(0xff30);
				...
				case 0xff4d:	// CPU speed
					...
					if (theGameboy.IsGbc)
					{
						if ((value & 0x01) != 0)
						{
							theGameboy.Cpu.CpuSpeed = 2;
							mem[address] = 0x80;
						}
						else
						{
							theGameboy.Cpu.CpuSpeed = 1;
							mem[address] = 0x7f;
						}
					}
		...
		internal void CopyMemoryToVram()
		{
			Array.Copy(mem, 0x8000, vRam, 0x2000 * vRamBankSelect, 0x2000);
		}

		internal void CopyMemoryToSaveRam()
		{
		...
		internal void CopyTo(byte[] dest, int address, int length)
		{
